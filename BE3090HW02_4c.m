...
while current_x < t
    ...
    sprimet= 6.94e-4  *step_size * .01 *(mistrad(i) / total(i))^-1; 
    sprimes = 6.94e-4*step_size * .01 *(missocial(i) / total(i))^-1; 
    sprimei = 6.94e-4 *step_size * .01 *(misindividual(i) / total(i))^-1; 
    ...
    sprimet= 6.94e-4  *step_size * .01 *((alltrad(i)) / total(i))^-1; 
    sprimes = 6.94e-4*step_size * .01 *((allsocial(i)) / total(i))^-1; 
    sprimei = 6.94e-4 *step_size * .01 *((allindividual(i)) / total(i))^-1; 
        precogt = 1 - precogt;
        precogs = 1 - precogs;
        precogi = 1 - precogi;
            new_trad = new_trad - precogt*kr*new_trad *step_size;
            new_social =  new_social - precogs*kr*new_social*step_size;
            new_individual = new_individual - precogi*kr*new_individual*step_size;  
    ...
    %newtraddist = newtraddist - precogt*kr*newtraddist*step_size;
    %newsocialdist = newsocialdist - precogs*kr*newsocialdist*step_size;
    %newinddist = newinddist - precogi*kr*newinddist*step_size;
...
figure;
hold on;
plot(x, finalA(end,:) / trapz(x, finalA(end,:)), 'r', "DisplayName", "Part A Distribution");
plot(x, inddists(end, :) /trapz(x, inddists(end, :)) , 'g', "DisplayName", "Part B Distribution");
...
avg1 = sum(x .*  finalA(end, :)) / sum( finalA(end, :));
avg2 =  sum(x .*  inddists(end, :)) / sum( inddists(end, :));
...
[h, p] = kstest2(finalA(end,: ), inddists(end,:));

% Check the result and print a message
if h == 0
    fprintf('The two samples are not significantly different (p = %.4f).\n', p);
else
    fprintf('The two samples are significantly different (p = %.4f).\n', p);
end